...
# Find the end of _doSquadCombat method (line 3393)
insert_marker = '        return { victory, defeated: allDefeated, escaped: !victory && !allDefeated, rounds, combatLog, monster, betrayed };\n    }\n\n    // 生成入侵勇者'

new_method = '''
    // 团队战斗：双方均可组队（最多3标准位+1临时位）
    // leftTeam: 勇者方角色数组, rightTeam: 怪物/敌方角色数组
    _doTeamCombat(leftTeam, rightTeam, options = {}) {
        ...
        // 叛变判定（如果leftTeam中有伪装者且rightTeam更强）
        let betrayed = false;
...
if insert_marker in content:
    content = content.replace(insert_marker, insert_marker.replace('    // 生成入侵勇者', new_method + '\n    // 生成入侵勇者'))
    with open('D:/KZ PROJECT/js/engine/Game.js', 'w', encoding='utf-8') as f:
        f.write(content)
    print('SUCCESS: _doTeamCombat added to Game.js')
else:
    print('ERROR: insert marker not found')
    # Debug
    idx = content.find('return { victory, defeated: allDefeated')
