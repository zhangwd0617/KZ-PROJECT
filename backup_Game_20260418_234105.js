...
class Game {
    ...
    _processHeroEncounters() {
        ...
        for (let i = 0; i < this.invaders.length; i++) {
            for (let j = i + 1; j < this.invaders.length; j++) {
                ...
                // 发起者决定行动
                let action = 'none';
                if (actor.talent[201] && target.talent[201]) {
                    // 双方都是伪装者（奴隶/间谍）：绝不内战，自发组队互助
                    action = 'spy_teamup';
                } else if (actor.talent[201]) {
                    // 伪装者对普通勇者执行破坏
                    action = 'disguise';
                } else if (target.talent[201]) {
                    // 普通勇者遇到伪装者：伪装者可能主动出手
                    action = 'disguise';
                ...
                if (action === 'combat') {
                    ...
                    // 伪装者执行破坏事件
                    const spy = actor.talent[201] ? actor : target;
                    ...
                    // 自发组队（设置相同小队ID）
                    const spySquadId = 900 + actor.id + target.id;
                    actor.cflag[900] = spySquadId;
                    target.cflag[900] = spySquadId;
                    this._markDailyEventTriggered(actor);
                    this._markDailyEventTriggered(target);
                    results.push({
                        title: `🛡️ 奴隶汇合`,
                        text: `${actor.name}与${target.name}都是魔王的仆从，相遇后互相掩护并恢复了${healAmt}HP，自发组成小队行动。`
                    });
                    ...
                    target.hp = Math.min(target.maxHp, target.hp + healAmt);
                    // 设置相同小队ID（使用一个独特的ID）
                    const newSquadId = 100 + actor.id + target.id;
                    actor.cflag[900] = newSquadId;
                    target.cflag[900] = newSquadId;
                    ...
                    results.push({
                        title: `🤝 初次组队`,
                        text: `${actor.name}与${target.name}初次相遇，一拍即合结为同伴！两人互相支援并恢复了${healAmt}HP，组成小队共同冒险。（关系：${this._getRelationLabel(this._getHeroRelation(actor, target).level)}）`
                    });
                } else if (action === 'help') {
                    // 莫逆之交：互相治疗/增益
    ...
    _confiscateFromLoser(winner, loser) {
        ...
        if (RAND(100) < 50 && loser.gear) {
            ...
            for (const slot of slots) {
                if (g[slot]) {
                    const item = g[slot];
                    // 转移给胜利者：如果槽位空则装备，否则放入背包
                    if (!winner.gear[slot]) {
                        winner.gear[slot] = item;
                    } else {
                        winner.gear.items = winner.gear.items || [];
                        winner.gear.items.push(item);
                    }
            ...
            if (g.weapons && g.weapons.length > 0) {
                for (const w of g.weapons) {
                    winner.gear.weapons = winner.gear.weapons || [];
                    winner.gear.weapons.push(w);
            ...
            if (takenNames.length > 0) {
                parts.push(`没收了装备：${takenNames.join('、')}`);
            }
        }
    ...
    _doSquadCombat(squad, monster) {
        ...
        while (rounds < maxRounds && monHp > 0) {
            ...
            // === 逃跑判定 ===
            const realAlive = aliveHeroes.filter(h => !h.talent[201]);
        ...
        if (!victory && !allDefeated) {
            const heroHp = survivors.filter(h => !h.talent[201]).reduce((s, h) => s + h.hp, 0);
            if (heroHp > monHp) {
                victory = true;
